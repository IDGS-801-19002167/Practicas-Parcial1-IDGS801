...
import forms, tolerancia, formAddColor
...
def resul():
    if request.method == "POST":
        ...
        if request.form.get("checkmulti"):
            num1 = request.form.get("n1")
            num2 = request.form.get("n2")
            return "La multiplicacion de {} X {} es = {}".format(
                num1, num2, str(int(num1) * int(num2))
            )
        ...
        if request.form.get("checkdiv"):
            num1 = request.form.get("n1")
            num2 = request.form.get("n2")
            return "La division de {} / {} es = {}".format(
                num1, num2, str(int(num1) // int(num2))
            )
...
def ecuacion():
    ...
    if request.method == "POST":
        ...
        resultC = (
            (int(number3) - int(number1)) ** 2 + ((int(number4) - int(number2))) ** 2
        ) ** 0.5
...
def agregarColores():
    coloradd = formAddColor.addColorForm(request.form)
    findcolor = formAddColor.searchColorForm(request.form)
    ...
    if request.method == "POST":
        if findcolor.buscar.data is None and coloradd.validate():
            ingles = coloradd.ingles.data
            espanol = coloradd.espanol.data

            colors = open("colores.txt", "a")
            colors.write(ingles + ":")
            colors.write(espanol + "\n")
            colors.close()

        elif findcolor.validate():
            buscar = findcolor.buscar.data
            lang = findcolor.lang.data

            colors = open("colores.txt", "r")
            for color in colors.readlines():
                color_data = color.split(":")
                if lang == "ingles" and color_data[0].strip() == buscar.strip():
                    busqueda = color_data[1].strip()
                    print(f"Color encontrado en inglés: {busqueda}")
                    break
                elif lang == "espanol" and color_data[1].strip() == buscar.strip():
                    busqueda = color_data[0].strip()
                    print(f"Color encontrado en español: {busqueda}")
                    break
